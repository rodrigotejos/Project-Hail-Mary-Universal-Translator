...
class TranslatorApp:  # pylint: disable=too-many-instance-attributes
    ...
    def create_main_layout(self):
        ...
        # Learning Module
        self.learning_text = ft.Text(
            "MÓDULO DE APRENDIZADO", color=THEME["accent_color"], size=16, weight="bold"
        )
        ...
        # Conversation Module
        self.conversation_text = ft.Text(
            "MÓDULO DE CONVERSAÇÃO", color=THEME["accent_color"], size=16, weight="bold"
        )
    ...
    def on_confirm_click(self, _e):
        """Handle confirm button click"""
        word = self.word_display.value.strip()
        if not word or word in ("NÃO DETECTADO", "ERRO"):
            return
    ...
    def play_last_audio(self, _e):
        ...
        if getattr(self, 'last_audio_file', None) and os.path.exists(self.last_audio_file):
            try:
                import playsound # pylint: disable=import-outside-toplevel, import-error
    ...
    def process_message(self, message: str, is_user: bool):
        ...
        for word in words:
            ...
            _clean_word = ''.join(c for c in word if c.isalnum())
            ...
            if result:
                translated_word, _confidence = result
