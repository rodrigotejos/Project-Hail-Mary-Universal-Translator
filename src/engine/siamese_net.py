...
class InterspeciesTripletDataset(Dataset):
    ...
    def __init__(self, data_dict, anchor_lang='ingles', virtual_size=500):
        ...
        data_dict format:
        {
            'CACHORRO': { 'ingles': ['path1'], 'elvish': ['path2'], 'klingon': ['path3'] }
        }
        """
        ...
        if len(self.classes) < 2:
            raise ValueError(f"É necessário pelo menos 2 palavras cadastradas em '{anchor_lang}' para treinar a rede siamesa.")
    ...
    def __getitem__(self, idx):
        ...
        pos_langs = [l for l in self.target_langs if l in self.data_dict[anchor_class] and len(self.data_dict[anchor_class][l]) > 0]
        if not pos_langs:
        ...
        neg_langs = [l for l in self.target_langs if l in self.data_dict[negative_class] and len(self.data_dict[negative_class][l]) > 0]
        if not neg_langs:
        ...
        # aplicamos Data Augmentation (Ruído/Volume) no Positivo para a IA não viciar no arquivo exato.
        is_same = False
...
class UniversalTranslatorSiameseNet(nn.Module):
    ...
    def __init__(self, embedding_dim=1024):
        super(UniversalTranslatorSiameseNet, self).__init__()
