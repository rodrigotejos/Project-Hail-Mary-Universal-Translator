"""
Script for training the Siamese Network locally.
"""
...
def train_universal_translator_model(epochs=10, batch_size=8, anchor_lang='ingles'):
    ...
    if TRAINING_MODE == "cloud":
        from engine.cloud_train import run_cloud_training # pylint: disable=import-outside-toplevel
        run_cloud_training(
            data_dict=data_dict,
            epochs=epochs,
    ...
    for epoch in range(epochs):
        ...
        print(f"=== Balanço da Época {epoch+1}/{epochs} | "
              f"Perda Contínua Estabilizada: {mean_loss:.4f} ===")
